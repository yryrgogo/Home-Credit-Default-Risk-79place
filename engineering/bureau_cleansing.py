...
def feature_contraction(data):
    ...
    for col in columns:
        ...
        #  data = contraction(data, col, limit, 0, 0)

        print(f'columns: {col}')
        #  print(data.query(f"{col} > {limit}").count())
        #  print(data.query(f"{col} < {limit}").count())

    data.to_csv('../data/bureau.csv', index=False)
    #  data.to_csv('../data/bureau_cleansing.csv', index=False)
    sys.exit()
...
def cats_replace(data):
    ...
    for cat in categorical:
        data[cat] = data[cat].map(lambda x: None if x is None else x)
...
def main():
    ...
    " データの読み込み "
    ' bureauのロード '
    #  data = pd.read_csv('../data/bureau_cleansing.csv')
    #  data = pd.read_csv('../data/bureau_cleansing.csv')
    data = pd.read_csv('../data/application_train_test.csv')

    #  data['over_flg'] = data['AMT_CREDIT_MAX_OVERDUE'].map(lambda x: 1 if x>-1 else 0)
    #  data['progress_flg'] = data['DAYS_ENDDATE_FACT'].map(lambda x: 0 if x<1 else 1)
    ...
    #  nan_list = ['CREDIT_DAY_OVERDUE', 'AMT_CREDIT_MAX_OVERDUE', 'CNT_CREDIT_PROLONG', 'AMT_CREDIT_SUM_OVERDUE', 'AMT_ANNUITY', 'AMT_CREDIT_SUM']
    #  data = set_nan(data, nan_list, 0)

    #  data.to_csv('../data/bureau_after.csv', index=False)
    #  data.to_csv('../data/bureau_cleansing.csv', index=False)
    #  sys.exit()
